...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const hasPermission = (moduleName: string, action: "view" | "edit" | "manage") => {
    ...
    // Check for exact match, then case-insensitive match, then label match
    let level = role.permissions[moduleName];
    
    if (!level) {
      // Try to find by label (e.g. "DASHBOARD" -> "Dashboard")
      const lowerModule = moduleName.toLowerCase().replace('_', ' ');
      const matchingKey = Object.keys(role.permissions).find(k => k.toLowerCase() === lowerModule);
      level = matchingKey ? role.permissions[matchingKey] : 'None';
    }
  ...
  const login = async (email: string, password?: string) => {
    try {
      ...
      // Mock login for user specified credentials
      if (email === "arifia.mulia@prasetia.co.id" && password === "Prasetia2025!@") {
        apiUser = {
          name: "Arifia Mulia",
          email: "arifia.mulia@prasetia.co.id",
          role: "admin",
          department: "Management",
          avatar: "https://api.dicebear.com/7.x/avataaars/svg?seed=Arifia"
        };
        token = "mock-token-for-arifia";
      } else {
