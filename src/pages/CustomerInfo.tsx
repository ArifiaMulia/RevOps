  ...
  const data = useMemo(() => {
    ...
    return {
      ...
      summary: {
        ...
        total_users: filteredDetails.reduce((sum, d) => sum + (d.total_users || 0), 0) || 6373, // Dynamic users or fallback
        kpis: {
           ...rawData.summary.kpis,
           l7d_arr_avg: l7dArr,
           arr_wow: arrWow,
           l7d_acc_avg: filteredDetails.length, // Constant for single customer
           acc_wow: 0
        }
      }
    };
       ...
       <CardContent className="p-5">
          ...
          <div className="flex items-end justify-between">
             <h2 className="text-2xl font-bold tracking-tight">
                {typeof value === 'number' && title.includes('ARR') ? formatCurrency(value) : value}
             </h2>
          ...
          <div className="h-10 w-full mt-2 opacity-30">
             <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data.risk_history.slice(-10)}>
                   <Area type="monotone" dataKey="total_arr" stroke={growth >= 0 ? "#10b981" : "#ef4444"} fill={growth >= 0 ? "#10b981" : "#ef4444"} fillOpacity={0.1} />
                </AreaChart>
             </ResponsiveContainer>
          </div>
       </CardContent>
