...
export const larkService = {
  ...
  // CRUD for Clients (mapped to details)
  addClient: (client: any) => {
    const current = larkService.getData();
    // Map client form data to Lark structure
    const newRecord = {
      account_name: client.name,
      Industry: client.industry,
      "Total Licenses Purchased": Number(client.licenses),
      "ARR (USD)": Number(client.arr.replace(/[^0-9.-]+/g, "")),
      "Renewal Date": client.renewal,
      "Churn Risk": client.health >= 80 ? "Low Churn Risk" : client.health >= 60 ? "Mid Churn Risk" : "High Churn Risk",
      // Add other fields as needed with defaults
      "% Adoption": client.health / 100, // Approximate
      "Duration": 0,
    ...
    const newDetails = [newRecord, ...current.details];
    
    // Update summary
    const newSummary = {
      ...current.summary,
      total_accounts: newDetails.length,
      total_arr: newDetails.reduce((sum, d) => sum + (d["ARR (USD)"] || 0), 0)
    };
  ...
  updateClient: (originalName: string, updates: any) => {
    const current = larkService.getData();
    const newDetails = current.details.map(d => {
      if (d.account_name === originalName) {
        return {
          ...d,
          account_name: updates.name,
          Industry: updates.industry,
          "Total Licenses Purchased": Number(updates.licenses),
          "ARR (USD)": Number(String(updates.arr).replace(/[^0-9.-]+/g, "")),
          "Renewal Date": updates.renewal,
          status: updates.status,
          // Recalculate risk if health is provided
          "Churn Risk": updates.health >= 80 ? "Low Churn Risk" : updates.health >= 60 ? "Mid Churn Risk" : "High Churn Risk",
          "% Adoption": updates.health / 100
        };
      }
      return d;
    });
  ...
  deleteClient: (accountName: string) => {
    const current = larkService.getData();
    const newDetails = current.details.filter(d => d.account_name !== accountName);
